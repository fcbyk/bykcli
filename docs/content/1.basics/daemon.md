...
description: 后台守护进程用于管理各插件以后台方式启动的进程
---
...
2. 输出访问 URL，并复制到剪贴板
3. Web 服务在后台子进程中持续运行
4. 在 `~/.bykcli/runtime/daemon/` 目录下记录 PID 文件
...
可能的输出:
- 正常终止：**`lansend(4252) -> terminated`**
- 进程已不在运行：**`lansend(4252) -> not_running`**
- 未找到对应 PID 文件：**`No managed process found with PID=4252.`**
